function softuniStudents(arr) {
    ...
    for (let line of arr) {
        if (line.includes(': ')) {
            ...
            let [userName, creditStr] = userInfo.split('[');
            let credits = Number(creditStr.replace(']', ''));
            let [email, courseName] = rest.split(' joins ');

            if (courses.has(courseName) && courses.get(courseName).capacity > 0) {

                courses.get(courseName).students.push({
                    username: userName,
                    email: email,
                    credits: credits
                });

                courses.get(courseName).capacity -= 1;

            }
        }
    }

    let sortedCourses = [...courses.entries()].sort((a, b) => b[1].students.length - a[1].students.length);

    for (let [courseName, courseData] of sortedCourses) {

        console.log(`${courseName}: ${courseData.capacity} places left`);
